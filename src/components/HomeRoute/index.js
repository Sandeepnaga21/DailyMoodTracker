...
class HomeRoute extends Component {
  state = {
    ...
    selectedDate: daysList[0].day,
  }

  onChangeEmojiName = event => {
    this.setState({selectedEmojiName: event.target.value})
  }
  ...
  render() {
    const {selectedEmojiName, selectedDate} = this.state
    return (
      <>
        <NavBar />
        <div className="h-container">
          <h1 className="h-head">Moods in a Month</h1>
          <div className="hl-container">
            <div className="calender">
              <ul className="u-list">
                {daysList.map(eachDay => (
                  <li className="l-list" key={eachDay.id}>
                    {eachDay.day}
                  </li>
                ))}
              </ul>
              <ul className="m-list">
                {initialMonthsList.map(eachMonth => (
                  <li className="l-list" key={eachMonth.month}>
                    <button type="button" className="button2">
                      {eachMonth.dates.map(eachDate => eachDate.date)}
                    </button>
                  </li>
                ))}
              </ul>
            ...
            <div className="s-container">
              <div className="em-container">
                <ul className="eu-list">
                  {emojisList.map(eachEmojy => (
                    <div className="c-container">
                      <li className="el-list" key={eachEmojy.id}>
                        {eachEmojy.emojiName}
                      </li>
                      <li key={eachEmojy.id} className="el-list">
                        <img className="em-img" src={eachEmojy.emojiUrl} />
                      </li>
                    </div>
                  ))}
                </ul>
              ...
              <div className="se-container">
                ...
                >
                  {emojisList.map(eachEmojy => (
                    <option>{eachEmojy.emojiName}</option>
                  ))}
                </select>
                <select
                  className="select"
        ...
        {emojisList.map(eachEmojy => (
          <ReportsRoute emojiDetails={eachEmojy} key={eachEmojy.id} />
        ))}
      </>
    )
  }
